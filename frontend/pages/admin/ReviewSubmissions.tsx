// src/pages/admin/ReviewSubmissions.tsx - Enhanced admin submission review
            ...
            <Textarea
              ...
              placeholder={
                actionType === 'approve' 
                  ...
                  : 'Explain why this submission needs changes...'
              }
              rows={4}
              className="w-full"
            />
...
export default function ReviewSubmissions() {
    ...
    try {
      if (actionType === 'approve') {
        ...
        toast({
          ...
          description: `"${selectedSubmission.title}" has been approved and is now live.`,
        });
      } else {
  ...
  const overdueCount = submissions.filter(s => {
    ...
    return s.status === 'pending' && daysSince > 7;
  }).length;
  ...
  return (
    <div className="space-y-6">
      ...
      {pendingCount > 0 && (
        <Alert className={overdueCount > 0 ? "border-red-500 bg-red-50" : "border-yellow-500 bg-yellow-50"}>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            You have <strong>{pendingCount}</strong> submission{pendingCount > 1 ? 's' : ''} waiting for review
            {overdueCount > 0 && (
              <span className="text-red-600">
                , including <strong>{overdueCount}</strong> overdue submission{overdueCount > 1 ? 's' : ''}
              </span>
            )}
            .
          </AlertDescription>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Review and moderate content submissions from critics
              </CardDescription>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Search submissions by title, creator, or content..."
                  value={searchQuery}
                        ...
                        {submission.admin_notes && (
                          <div className="bg-gray-50 p-3 rounded-lg mb-3">
                            <p className="text-sm text-gray-600">
                              <strong>Previous Notes:</strong> {submission.admin_notes}
                            </p>
                          </div>
                        )}

                        {/* Performance Stats for Approved Content */}
